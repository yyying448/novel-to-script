...
export function CompareView({ results, viewModel }: any) {
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{results.map((r:any,i:number)=>{const s=r.score||{};const t=s.总分||Object.values(s).reduce((a:any,b:any)=>a+b,0)||0;const b=t>=max&&t>0;return(<div key={i} className={`bg-gray-50 border-2 rounded-2xl p-5 ${b?"border-black":"border-gray-200"}`}><h3 className="font-bold text-sm mb-3 text-black">{b?"★ ":""}{r.label}</h3><div className="text-xs space-y-1 text-gray-500"><div>{r.scene_count}场</div><div>评分 {JSON.stringify(s)}</div>{r.error?<div className="text-red-500">! {r.error}</div>:null}</div>{r.all_scenes?.length>0?<button onClick={()=>viewModel(r.label)} className={`${btn} mt-3 !text-xs !px-3 !py-1.5 border border-gray-300 text-gray-600 hover:bg-gray-100`}>查看完整</button>:null}</div>)})}</div></div>
}

export function RevisePanel({ scriptResult, revChapter, setRevChapter, revFeedback, setRevFeedback, handleRevise, handleVisualize, revising, vizLoading, vizResult, setVizResult }: any) {
  const chs=[...new Set((scriptResult.scenes||[]).map((s:Scene)=>s.chapter).filter(Boolean))] as string[]
  return <div className={card}><h2 className="text-base font-bold text-black mb-5">修改剧本</h2>
    {revising && <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl text-sm text-center text-gray-600">⏳ 正在调用 AI 修改 {revChapter}，请稍候...</div>}
    <div className="flex gap-3 mb-4 items-center"><label className="text-sm text-gray-500 font-medium">章节</label><select value={revChapter} onChange={e=>setRevChapter(e.target.value)} className="px-3 py-1.5 bg-white border border-gray-300 rounded-full text-sm text-black outline-none">{chs.map((c:string)=><option key={c} value={c}>{c.length>15?c.slice(0,15)+"…":c}</option>)}</select></div><textarea value={revFeedback} onChange={e=>setRevFeedback(e.target.value)} placeholder="描述需要修改的内容..." className="w-full h-24 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-black resize-y outline-none placeholder:text-gray-400"/>
    {vizResult && <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-xl text-sm max-h-48 overflow-y-auto"><p className="text-xs text-gray-500 mb-2 font-medium">视觉化预览：</p><pre className="text-xs text-gray-800 whitespace-pre-wrap">{vizResult}</pre><button onClick={()=>setVizResult("")} className="mt-2 px-3 py-1 border border-gray-300 rounded-full text-xs text-gray-500">关闭预览</button></div>}
    <div className="flex gap-2 mt-4">
      <button onClick={handleRevise} disabled={revising||vizLoading} className={`${btn} bg-black text-white hover:bg-gray-800 disabled:opacity-50`}>{revising?"修改中...":"应用修改"}</button>
      <button onClick={handleVisualize} disabled={revising||vizLoading} className={`${btn} border border-gray-300 text-gray-600 hover:bg-gray-50`}>{vizLoading?"视觉化中...":"🎬 视觉化改写"}</button>
    </div></div>
}
